...
def sampling(args):
    ...
    eps = K.random_normal((tf.shape(mean)[0], tf.shape(mean)[1]))
...
def run_vae_few_shot(df: pd.DataFrame, progress_callback: Callable[[str], None] = print) -> pd.DataFrame:
    """
    VAE-based anomaly detection with zero-shot classification.
    Prints progress at each major step and sends via progress_callback.

    Returns DataFrame with original columns plus:
      - anomaly_score: reconstruction MSE
      - predicted_label: "Normal" or "Anomaly"
      - threshold: the 95th percentile MSE on training normal samples
    """
    def log(msg: str):
    ...
    vae = load_vae("vae_model.keras", X_scaled.shape[1])
    if vae is None:
    ...
    log("[4/6] Computing reconstruction errors and threshold...")
    ...
    for idx, flag in enumerate(flags):
        if flag:
            ...
            text = (
                ...
                f"{int(row['MINFLT'])} minor faults, {int(row['MAJFLT'])} major faults, {mem:.1f}% memory."
            )
            res = classifier(
                text,
